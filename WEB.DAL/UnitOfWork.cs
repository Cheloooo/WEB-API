...
public class UnitOfWork : IUnitofWork
{
    ...
    public Task ExecuteAsync(Func<DbConnection, DbTransaction, CancellationToken, Task> work,
        CancellationToken ct = default)
        => ExecuteCoreAsync<object?>(async (_, __, c) =>
        {
            await work(c);
            return null;
        }, ct);
    ...
    private async Task<TResult> ExecuteCoreAsync<TResult>(Func<DbConnection, DbTransaction, CancellationToken, Task<TResult>> work, CancellationToken ct)

    public Task<TResult> ExecuteAsync<TResult>(Func<DbConnection, DbTransaction, CancellationToken, Task<TResult>> work,
        CancellationToken ct = default)
        => ExecuteCoreAsync(async (_, __, c) =>
        {
            await work(c);
            return null;
        }, ct);
   public Task ExecuteCoreAsync(Func<DbConnection, DbTransaction, CancellationToken, Task> work, CancellationToken ct = default)
   => ExecuteCoreAsync<object?>(async (cnn, tx, c) =>
   {
       await work(cnn, tx, c);
       return null;
   }, ct);

   public Task<TResult> ExecuteAsync<TResult>(Func<CancellationToken, Task<TResult>> work,
       CancellationToken ct = default)
       => ExecuteCoreAsync(work, ct);

   public Task ExecuteReadOnlyAsync(Func<DbConnection, DbTransaction, CancellationToken, Task> work,
       CancellationToken ct = default)
       => work(ct);

   public Task<TResult> ExecuteReadOnAsync<TResult>(Func<CancellationToken, Task<TResult>> work,
       CancellationToken ct = default)
       => work(ct);
